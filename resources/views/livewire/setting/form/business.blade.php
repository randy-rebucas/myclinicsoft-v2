...
use function Livewire\Volt\{state, form, mount, usesFileUploads};
    ...
    <x-card>
        <x-slot name="header">
            ...
            <p class="mt-1 text-sm text-gray-600">
                {{ __("Update your account's business information.") }}
            </p>
        </x-slot>
            ...
            <div class="space-y-4 p-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
                    <div class="md:col-span-3">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <input type="file" wire:model.live="logo" id="logo" class="hidden"
                                    accept="image/*" />
                ...
                @foreach (['business_name' => 'Business Name', 'address' => 'Business Address', 'contact_number' => 'Contact Number', 'email' => 'Email Address'] as $field => $label)
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
                        ...
                        <div class="md:col-span-3">
                            @if ($field === 'address')
                                <x-textarea wire:model.live.blur="form.settings.{{ $field }}"
                                    id="{{ $field }}" :placeholder="__('Enter ' . strtolower($label))" class="w-full" rows="3" />
                            @else
                                <x-text-input wire:model.live.blur="form.settings.{{ $field }}"
                                    id="{{ $field }}" :placeholder="__('Enter ' . strtolower($label))" class="w-full" :type="$field === 'email' ? 'email' : 'text'" />
                            @endif
